...
- Dynamic position sizing (reduce on drawdown)
- Trailing stops (protect gains)
- Market correlation filter (S&P500 5-day trend)
- Inverse pyramiding (reduce on correlated losses)
- ATR dynamic stops
- Bear/sector filters
"""
import json
...
def score_ticker(closes, highs, lows, volumes, regime, universe_returns=None):
    ...
    score += 15.0  # fundamental placeholder
...
def market_trend_5d(bench_closes):
    ...
    if len(bench_closes) < 6:
        return True
    return bench_closes[0] > bench_closes[5]
...
def run_backtest():
    ...
    # Best by Sharpe (balance perf + risk)
    best_sharpe = max(results, key=lambda x: x["sharpe"])
    best_return = max(results, key=lambda x: x["total_return"])
    best        = best_sharpe  # primary: best risk-adjusted
    ...
    rows = [
        ...
        ("Market 5d Filter", "✅ No entry if falling", "✅ No entry if falling", ""),
    ...
    for c, h in enumerate(["Ticker","Sector","Entry","Exit","Entry$","Exit$","Trail High","P&L%","Reason","Regime","Days"], 1):
    ...
    for r_idx, t in enumerate(sorted(best["trades"], key=lambda x: x["entry_date"]), 2):
        vals = [t["ticker"], t.get("sector",""), t["entry_date"], t["exit_date"],
                t["entry_price"], t["exit_price"], t.get("trail_high",""),
    ...
    for c, h in enumerate(["Date","Best Sharpe ($)","Best Return ($)","S&P500 ($10k)"], 1):
        ws5.cell(row=1, column=c, value=h)
    bench_norm = bench_df["close"] / bench_df["close"].iloc[0] * INITIAL_CASH
    eq_return  = best_return["equity"]
    for i, (d, e_s) in enumerate(zip(all_dates, best_sharpe["equity"]), 2):
        b = bench_norm.get(d)
        e_r = eq_return[i-2] if i-2 < len(eq_return) else None
        ws5.cell(row=i, column=1, value=str(d))
        ws5.cell(row=i, column=2, value=e_s)
        ws5.cell(row=i, column=3, value=e_r)
        ws5.cell(row=i, column=4, value=round(float(b), 2) if b is not None else None)

    chart = LineChart()
    chart.title = "Model (Sharpe) vs Model (Return) vs S&P500"
    chart.style = 10
    n_rows = len(best_sharpe["equity"]) + 1
    chart.add_data(Reference(ws5, min_col=2, max_col=4, min_row=1, max_row=n_rows), titles_from_data=True)
    chart.width = 30; chart.height = 18
    ws5.add_chart(chart, "F2")
